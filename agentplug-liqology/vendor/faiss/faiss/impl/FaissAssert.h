...
// Exceptions unavailable on this target (e.g. wasm32-wasip1-threads with no
// exception-capable libc++abi): a FAISS_THROW_* call means the caller-side
// precondition checks (DisambiguationIndex::add/query) already failed to
// catch a bad input before it reached FAISS -- abort loudly rather than
// silently corrupt state, matching FAISS_ASSERT's own abort() behavior.
#define FAISS_THROW_MSG(MSG)                                                                \
    do {                                                                                    \
        fprintf(stderr, "FaissException (aborting, no exceptions): %s at %s:%d\n", MSG,     \
                __FILE__, __LINE__);                                                        \
        abort();                                                                            \
    } while (false)
...
#define FAISS_THROW_FMT(FMT, ...)                                                              \
    do {                                                                                        \
        fprintf(stderr, "FaissException (aborting, no exceptions): " FMT " at %s:%d\n",         \
