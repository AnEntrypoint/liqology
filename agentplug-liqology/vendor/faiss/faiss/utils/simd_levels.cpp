...
#else
            fprintf(stderr, "FaissException (aborting, no exceptions): Invalid SIMDLevel\n");
            abort();
#endif
    }
}
...
#else
    fprintf(stderr, "FaissException (aborting, no exceptions): Invalid SIMD level string: %s\n",
            level_str.c_str());
    abort();
#endif
}
